...
public class YahooFinanceExchangeAPI extends ExchangeAPI {
	...
	public HashMap<String, Double> queryRates(HashSet<String> fromCurrencies,
		...
		try {
			...
			throw new Exception(
					"Failed to query Yahoo Finance -- check your network.");
		}
	...
	private void checkFormat(HashSet<String> currencies) throws Exception {
		for (String currency : currencies) {
			if (currency.length() != 3)
				throw new Exception("Wrong code: \"" + currency
						+ "\". Yahoo Finance uses three-letter currency codes.");
		}
	}

	private String queryYahooFinance(HashSet<String> fromCurrency,
			HashSet<String> toCurrency) throws Exception {
	...
	private void putRateItem(JsonNode rateItem, HashMap<String, Double> buffer)
			throws Exception {
		buffer.put(rateItem.findValue("id").asText().subSequence(0, 3)
				.toString()
				+ "/"
				+ rateItem.findValue("id").asText().subSequence(3, 6)
						.toString(),
				Double.valueOf(rateItem.findValue("Rate").asText()));
	}

	private HashMap<String, Double> parseJSONYahooFinanceResponse(
			String response) throws Exception {

		HashMap<String, Double> buffer = new HashMap<>();

		JsonNode ratesRoot = new ObjectMapper()
				.readValue(response, JsonNode.class).findValue("query")
				.findValue("results").findValue("rate");

		if (ratesRoot.isObject()) {
			putRateItem(ratesRoot, buffer);
		} else if (ratesRoot.isArray()) {
			for (JsonNode rateItem : ratesRoot)
				putRateItem(rateItem, buffer);
		}
